...
- **Campos:**
  ...
  - `material` (opcional, parte do form-data) — `glass`, `opaque` ou `auto`. Decide se o refinador aplica vidro PBR ou preserva a textura pintada pela IA. Omitido ou `auto` deixa a decisão com o `ClipTransparencyClassifier` — que **erra**, e é o motivo do campo existir (ver [17](17-fidelidade-do-modelo.md#defeito-1--frasco-opaco-classificado-como-vidro)). Valor desconhecido devolve **422**.
  - `labelBox` (opcional, parte do form-data) — `x,y,w,h` normalizados em `[0,1]` **relativos à foto frontal inteira**, marcando onde fica a label. Quando enviado, o backend pula o detector automático e projeta exatamente essa região; omitido, o `HomographyLabelExtractor` procura sozinho e desiste em silêncio se não achar nada confiável. Existe porque frasco com o nome gravado direto no vidro não tem região para segmentar (ver [09e](09e-aplicacao-label.md#marcação-manual-labelbox)). Devolve **422** se não tiver 4 números, se algum sair de `[0,1]`, se o lado for menor que 2% ou se `x+w`/`y+h` ultrapassar a borda.

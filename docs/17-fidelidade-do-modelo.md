...
- **`HUNYUAN_TEXTURE_RESOLUTION` é ignorada no pipeline instalado.** Confirmado em 2026-08-16: `Hunyuan3DPaintPipeline.__call__` aceita somente `(mesh, image)`. A tentativa multi-view falha porque recebe uma lista (`'list' object has no attribute 'mode'`), cai para a primeira foto e `_filtrar_kwargs()` omite `texture_resolution`. Mudar apenas `1024 → 2048` não altera a execução atual. A geometria continua multi-view em `octree=384`. Ver a [auditoria de configuração do Hunyuan](09b-pipeline-ai-hunyuan.md#auditoria-da-configuração-local-em-2026-08-16).
